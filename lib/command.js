  ...
  if (major === 'lists') {
      ...
      if (err) {
        ...
        for (i in result.rows) {
          if (Object.hasOwnProperty.call(result.rows, i)) {
            console.log(
              result.rows[i].name + ' (' + result.rows[i].totalTasks + ')'
            )
          }
        }
      }
    ...
    this.task.getDone(minor, function (err, result) {
      ...
      if (err) {
        ...
        for (i in result.rows) {
          if (Object.hasOwnProperty.call(result.rows, i)) {
            var doneString =
              '(' + result.rows[i].id + ') ' + result.rows[i].name + ' | '
    ...
    this.task.get(minor, function (err, result) {
      if (err) {
        ...
        console.log(
          'Due Date: ' +
            (result.row.date ? self.parseDate(result.row.date) : 'none')
        )
        console.log('Done: ' + (result.row.done ? 'yes' : 'no'))
        console.log(
          'Done Date: ' +
            (result.row.done_date
              ? self.parseDate(result.row.done_date)
              : 'none')
        )
      ...
      for (i in result.rows) {
        if (Object.hasOwnProperty.call(result.rows, i)) {
          if (currentList !== result.rows[i].listName) {
            ...
            console.log(
              result.rows[i].listName.bold.underline + ':'.bold.underline
            )
          }
        }
...
Command.prototype.showHelp = function () {
  ...
  console.log('')
  console.log(
    '    nodo star <task_id>              Star a task (will display with different color)'
  )
