...
Task.prototype.getToDo = function (callback) {
  var query =
    'SELECT lists.name as listName, tasks.name, tasks.id, tasks.important '
...
Task.prototype.getDone = function (limit, callback) {
  var query =
    'SELECT lists.name as listName, tasks.* FROM tasks, lists WHERE done=1 '
  query +=
    'AND tasks.deleted=0 AND tasks.list_id=lists.id ORDER BY tasks.done_date DESC'
  if (limit) {
    query += ' LIMIT ' + limit
  }
  database.all(query, function (err, rows) {
...
Task.prototype.add = function (task, callback) {
  database.get('SELECT id FROM lists WHERE name=?', task.list, function (
    err,
    row
  ) {
    if (err) {
...
Task.prototype.move = function (taskId, listName, callback) {
  database.get('SELECT id FROM lists WHERE name=?', listName, function (
    err,
    row
  ) {
    if (err) {
